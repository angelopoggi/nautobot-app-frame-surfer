...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        ...
        shutil.copy(
            os.path.join(
                context.frame_surfer.compose_dir, "creds.example.env"
            ),
            os.path.join(context.frame_surfer.compose_dir, "creds.env"),
        )
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.frame_surfer.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.frame_surfer.python_ver}"
            run_command(context, command)
    else:
